...
function renderProducts(category = 'all', searchTerm = '') {
    ...
    // Adjust container classes for carousel behavior when on mobile
    if (isMobile) {
      productGrid.classList.remove('flex-col');
      productGrid.classList.add('flex-row','overflow-x-auto','scroll-smooth','snap-x','snap-mandatory','touch-pan-x');
    } else {
      productGrid.classList.remove('flex-row','overflow-x-auto','scroll-smooth','snap-x','snap-mandatory','touch-pan-x');
      productGrid.classList.add('flex-col');
    }
    filteredProducts.forEach((product, index) => {
      ...
      if (isMobile) {
        // Make each slide full width for carousel
        productCard.className = 'group bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 animate-on-scroll w-full min-w-full flex-shrink-0 snap-start mx-0 cursor-pointer flex flex-col';
      } else {
        // Desktop: slightly larger, squarer cards
        productCard.className = 'group bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 animate-on-scroll min-w-[280px] w-[320px] flex-shrink-0 snap-start cursor-pointer flex flex-col';
      }
        ...
        productCard.innerHTML = `
            <div class="relative overflow-hidden h-40 md:h-72 bg-gray-50">
    ...
    // After rendering slides, if mobile enable range / buttons control
    if (isMobile) {
      setupMobileCarousel(filteredProducts.length);
    } else {
      resetMobileCarousel();
    }
  ...
  function setupMobileCarousel(slideCount) {
    const grid = document.getElementById('product-grid');
    const range = document.getElementById('mobileCarouselRange');
    const prev = document.getElementById('mobilePrevBtn');
    const next = document.getElementById('mobileNextBtn');
    if (!grid || !range) return;

    // configure range
    range.max = Math.max(0, slideCount - 1);
    range.value = 0;

    // ensure proper classes on children (safety)
    Array.from(grid.children).forEach(child => {
      child.classList.add('min-w-full','w-full','flex-shrink-0','snap-start');
    });
    ...
    // update range while user scrolls (debounced)
    grid.onscroll = () => {
      clearTimeout(_mobileCarousel.onScrollTimer);
      _mobileCarousel.onScrollTimer = setTimeout(() => {
    ...
    if (prev) prev.onclick = () => {
      const v = Math.max(0, parseInt(range.value, 10) - 1);
      range.value = v; range.oninput({ target: range });
    };
    if (next) next.onclick = () => {
      const v = Math.min(range.max, parseInt(range.value, 10) + 1);
      range.value = v; range.oninput({ target: range });
    };

    _mobileCarousel.active = true;
  }
  ...
  function resetMobileCarousel() {
    ...
    if (grid) {
      grid.onscroll = null;
      Array.from(grid.children).forEach(child => {
        child.classList.remove('min-w-full','w-full','flex-shrink-0','snap-start');
      });
    }
    if (prev) prev.onclick = null;
    if (next) next.onclick = null;
    _mobileCarousel.active = false;
  }
